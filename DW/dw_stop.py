...
from kki_func import slack
from kki_func import blynk
...
# ボタンの指定ピン
V_PINS = [PIN, PIN1, PIN2, PIN3, PIN4, PIN5, PIN8, PIN9]
# LCDの指定ピン
LCD = ["V6", "V7"]
...
def driver_get():
    ...
    driver = webdriver.Chrome(ChromeDriverManager().install(), service=service, chrome_options=chop)
...
def main():
    ...
    blynk.lcd_write(blynk_token, LCD, '停 止 要 因 入 力', '待 機 中 。。。')
    ...
    try:
        ...
        while True:
            # 全てのボタンの状態を取得
            buttons = blynk.pins_read(blynk_token, V_PINS)
            print(f'start---------------------------')
            # ONになっているボタンのみ取得
            ...
            val = blynk.pin_read(blynk_token, PIN9)
            if val == 1:
            ...
            if PIN8 in keys.keys():
                ...
                if keys[PIN8] == 1 and before_key == PIN5:
                    slack.slack_post(slack_token, channel,
                               '操作ミスのようです。荷待ちは発生していません:pray:')
                    blynk.lcd_write(blynk_token, LCD, 'リセットボタンが押されたため', '解除します。')
                    flg = 0
                    cnt = 0
                    time.sleep(3)
                    blynk.pins_clear(blynk_token, V_PINS)
                    blynk.lcd_write(blynk_token, LCD, '停 止 要 因 入 力', '待 機 中 。。。')
                    continue
                elif keys[PIN8] == 1:
                    blynk.lcd_write(blynk_token, LCD, 'リセットボタンが押されたため', '解除します。')
            ...
            if status == 1 and flg == 0:
                ...
                blynk.pins_reset(blynk_token, V_PINS, before_key)
            ...
            if flg == 1 and cnt == 0:
                ...
                blynk.lcd_write(blynk_token, LCD, f'{start_time}:{stop_name}')
                cnt = 1
        ...
        blynk.lcd_write(blynk_token, LCD, 'システムを終了します。')
        time.sleep(4)
        blynk.lcd_write(blynk_token, LCD)
        blynk.pins_clear(blynk_token, V_PINS)
